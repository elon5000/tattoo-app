...
export default function MainNavBar(props: Props) {
    ...
    const navStyles = `flex-between cursor-default ${(!isAboveMediumScreen && isMainNavOpen) ?
        'py-12 flex-col h-full absolute w-3/5 bg-primary-100 top-0 end-0 animate-slide-in-left'
        : 'mx-auto w-full'}`
    const navUlStyles = isAboveMediumScreen ? 'flex-between gap-8 mx-auto w-1/2 text-sm'
        : 'flex-between flex-col gap-8 text-xl'

    const toggleModalBtnClassStr = `w-12 p-2 hover:text-primary-300 absolute ${isMainNavOpen ? 'start-0' : 'end-0 rounded-full bg-primary-500 text-primary-100 hover:bg-zinc-950/90'}`
